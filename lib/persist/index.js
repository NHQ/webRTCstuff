...
module.exports = function(db){
  ...
  var o = {
    ...
    setRoom:function(room,cb){
      ...
      if(room.data.id && room.data.id.indexOf('_') === -1){
        room.data.id = room.data.id.split('_').pop();
      }

      var pick = function(data){
        data =  _.pick(data,['id','type','director','data','title','description','active','videos','members','visitors','_rev']);
        if(data.id && data.id.indexOf('_')) {
        }
      ...
      queuedInsert(db,room.getData(),'room_'+id,function(err,data){
    ...
    getRoom:function(id,cb){
      db.get('room_'+id,function(err,data){
        ...
        console.log('GETTING ROOM FROM db  ',data);
        process.exit();
        cb(null,room);
      });
    },
